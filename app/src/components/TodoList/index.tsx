...
export default function TodoList() {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Undo toast */}
      {undoItem && (
        <div className="mx-4 mb-2 px-4 py-2 bg-gray-800 dark:bg-gray-200 text-white
                        dark:text-gray-800 rounded-lg text-sm flex items-center justify-between slide-up">
          <span>已删除「{undoItem.todo.title}」</span>
          <button
            onClick={undoDelete}
            className="flex items-center gap-1 text-tomato font-medium hover:underline"
          >
            <Undo size={14} />
            撤销
          </button>
